...
### Performance Management Loop (rater observations, counseling, and traceability)
...
- ✅ Supabase Postgres hosted in the `aws-1-us-east-2` (US) region for reduced latency; connection pooling tuned to the Shared Pooler's actual session cap.
- ✅ Node.js 22 required across both apps (`.nvmrc` plus a launcher script that forces npm scripts onto Node 22 even from a Node 20 shell).
